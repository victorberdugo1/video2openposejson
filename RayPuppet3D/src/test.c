...
void DrawUI(GameWorld* w) {
	...
	DrawText("J: Jump", 10, 290, 14, DARKGREEN);
	DrawText("K: Kick", 10, 310, 14, DARKGREEN);
	DrawText("P: Punch", 10, 330, 14, DARKGREEN);
	...
	if (w->controlled >= 0) {
		...
		if (c->isPlayingAction) {
			...
			DrawText("[ACTION PLAYING]", 10, 360, 16, RED);
			DrawText(TextFormat("Frame: %d/%d", currentFrame, totalFrames), 10, 380, 14, RED);

			float progress = (float)currentFrame / (float)totalFrames;
			DrawRectangle(10, 400, 200, 10, LIGHTGRAY);
			DrawRectangle(10, 400, (int)(200 * progress), 10, RED);
		}
	}
}
