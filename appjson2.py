...
# moviepy 2.x: ya no existe moviepy.editor, se importa directamente desde moviepy
...
# Lista de keypoints de MediaPipe para el JSON
MEDIAPIPE_KEYPOINTS = [
...
def get_frames(video_in):
    ...
    # moviepy 2.x: resize → resized(height=512)
    clip_resized = clip.resized(height=512)
    clip_resized.write_videofile("video_resized.mp4", fps=target_fps, logger=None)
...
def process_frame(frame_path):
    ...
    if landmarks:
        ...
        for idx, mx in enumerate(mean_xs):
            if mx < 0.5 and "side" not in assigned:
                assigned["side"] = idx
            elif mx >= 0.5 and "front" not in assigned:
                assigned["front"] = idx
        if "side" not in assigned:
            assigned["side"] = int(np.argmin(mean_xs))
        if "front" not in assigned:
            assigned["front"] = int(np.argmax(mean_xs))
        for key, pid in [("side", "person_0"), ("front", "person_1")]:
            ...
            for idx, name in MEDIAPIPE_KEYPOINTS:
                if idx < len(lm):
                    ...
                    person[name] = {
                        ...
                        'visibility': float(getattr(v, "visibility", 1.0))
                    }
                else:
            ...
            if 11 < len(lm) and 12 < len(lm):
                l, r = lm[11], lm[12]
                person["Neck"] = {
                    'x': float((l.x + r.x) / 2), 'y': float((l.y + r.y) / 2),
                    'z': float((l.z + r.z) / 2),
...
def convert_gif_to_video(gif_file):
    # gradio 5.x: gr.File devuelve la ruta directamente (str), no un objeto con .name
    gif_path = gif_file if isinstance(gif_file, str) else gif_file.name
    clip = VideoFileClip(gif_path)
    clip.write_videofile("gif_video.mp4", logger=None)
...
def infer(video_in):
    ...
    for frame_key in all_kps:
        ...
        if "person_0" in frame_data and "person_1" in frame_data:
            ...
            torso_center_x = (
                (side_person["LShoulder"]["x"] + side_person["RShoulder"]["x"]) / 2
                if "LShoulder" in side_person else 0.5
            )
            combined_person = {}
...
def clean_files():
    for f in os.listdir():
        if os.path.isfile(f) and (
            f.startswith('frame_') or f.startswith('openpose_') or f == "video_resized.mp4"
        ):
            os.remove(f)
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            # gradio 5.x: sources sigue siendo una lista, sin cambios aquí
            video_in = gr.Video(sources=["upload"])
            # gradio 5.x: gr.File devuelve str (ruta) en lugar de objeto con .name
        ...
        with gr.Column():
            ...
            # gradio 5.x: gr.Files sigue existiendo para múltiples archivos de salida
